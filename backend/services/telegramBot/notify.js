...
const DEFAULT_TEXTS = {
  ...
  // {reward} — уже готовая строка вида «7 дн. подписки» или «150 ₽ на баланс»:
  // тип начисления известен только в момент активации, и собирать её шаблоном
  // из двух взаимоисключающих полей было бы неудобно админу.
  user_promo_applied:
  ...
  // Автопилот рассылок. Приходит ДО отправки: это единственное окно, в
  // которое человек может вмешаться — у API бота отмены нет.
  admin_broadcast_scheduled:
    ...
    '{text}\n\n' +
    'Отменить можно в админке: Bedolaga Bot → Рассылка сообщений → ИИ.',
  admin_payment_received:
    '💰 <b>Платёж получен</b>\nЮзер: {login}\nСумма: {amount} ₽\nТариф: {plan}',

  // Деньги взяты, а доступа в панели нет. Самое неприятное из того, что может
  // случиться тихо: снаружи всё выглядит успешно — платёж прошёл, в кабинете
  // висит активная подписка, — и без этого сообщения о проблеме узнают только
  // от самого человека, когда он не сможет подключиться.
  admin_provisioning_failed:
    ...
    'Причина: {error}\n\n' +
    '{next}',
  // Активация упала целиком: деньги приняты, а подписки нет вообще — чинить
  // крону нечего, в базе нет строки. Нужен человек.
