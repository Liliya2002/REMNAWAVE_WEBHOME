...
router.post('/activate', verifyToken, verifyActive, async (req, res) => {
  try {
    const { activateTrial } = require('../services/trialActivation')
    const r = await activateTrial(req.userId)
    if (!r.ok) return res.status(400).json({ error: r.error })
    res.json({ success: true, subscription: r.subscription })
  } catch (err) {
    console.error('[Subscriptions] Error activating free trial:', err)
    res.status(500).json({ error: 'Ошибка при активации бесплатного периода' })
  }
})
