  ...
  return (
    <div className="space-y-6">
      <div className="p-4 sm:p-8 bg-gradient-to-br from-slate-800/40 to-slate-900/50 border border-slate-700/50 rounded-2xl">
        <h3 className="text-xl sm:text-2xl font-bold text-white mb-6">Баланс</h3>
        ...
        {loading ? (
          <div className="text-slate-400 py-8">Загрузка раздела баланса...</div>
        ) : (
          <>
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 mb-6">
              <div className="rounded-2xl border border-cyan-500/30 bg-[radial-gradient(circle_at_20%_15%,rgba(34,211,238,0.2),transparent_38%),rgba(2,6,23,0.85)] p-6">
                <div className="text-xs uppercase tracking-[0.16em] text-cyan-300 mb-2 inline-flex items-center gap-2"><Wallet className="w-4 h-4" /> Текущий баланс</div>
                <div className="text-4xl font-extrabold text-white">{balance.toFixed(2)} {currency === 'RUB' ? '₽' : currency}</div>
                <div className="text-xs text-slate-400 mt-2">Средства можно использовать для оплаты подписок напрямую.</div>
              </div>

              <div className="rounded-2xl border border-slate-700/60 bg-slate-900/40 p-6">
                <div className="text-sm text-slate-300 mb-4 inline-flex items-center gap-2"><CreditCard className="w-4 h-4 text-blue-400" /> Пополнение баланса</div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs text-slate-400 block mb-1">Платежная система</label>
                    <select
                      value={paymentSystem}
                  ...
                  <div>
                    <label className="text-xs text-slate-400 block mb-1">Сумма</label>
                    <input
                      type="number"
                      min="10"
                      step="10"
            ...
            <div className="rounded-2xl border border-slate-700/60 bg-slate-900/40 p-6">
              <h4 className="text-lg font-semibold text-white mb-4 inline-flex items-center gap-2"><History className="w-4 h-4 text-cyan-300" /> История операций</h4>
              {operations.length === 0 ? (
                <div className="text-sm text-slate-400">Операции отсутствуют.</div>
              ) : (
                <div className="space-y-2">
                  {operations.map((op) => (
                    <div key={op.id} className="rounded-lg border border-slate-700/50 bg-slate-950/40 p-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                      <div>
                        <div className="text-sm text-slate-100 font-medium">{operationLabel(op)}</div>
                        <div className="text-xs text-slate-500">{formatDate(op.created_at)}</div>
                      </div>
                      <div className="text-right">
                        <div className={`font-bold ${op.direction === 'out' ? 'text-rose-300' : 'text-emerald-300'}`}>
                          {op.direction === 'out' ? '-' : '+'}{Number(op.amount || 0).toFixed(2)} ₽
                        </div>
                        <div className="text-xs text-slate-500">{op.status || 'completed'}</div>
                      </div>
                    </div>
                  ))}
                </div>
