...
**Оплачено ≠ выдано.** `activateSubscription` писала подписку в базу
независимо от того, создался ли пользователь в RemnaWave. Если панель не
отвечала или у тарифа был пустой `squad_uuids`, в базе появлялась строка с
`is_active = true` и `remnwave_user_uuid = NULL`: человек заплатил, видит
активную подписку и не может подключиться, а мы об этом не узнавали — активация
не падала, вебхук отвечал 200. Теперь есть `provisioning_status`
(`ok`/`pending`/`failed`), `services/provisioning.js` с повторами
(`cron/provisioningRetry.js`, 10 попыток за ~11 часов) и уведомление
`admin_provisioning_failed`. `is_active` при этом НЕ снимается: подписка
оплачена и существует, не выдан только доступ — сняв флаг, мы покажем
заплатившему «подписки нет» и разрешим ему взять пробник поверх неё. Тариф без
серверной группы больше не сохраняется активным (`routes/plans.js`).
...
- Логи — через `services/logger.js` (`logger.for('Раздел')`). Голый `console.*`
  тоже работает: `logger.install()` в `index.js` оборачивает его и добавляет
  время с уровнем. В лог не должны попадать тело запроса, заголовки и
  query-строка — там пароли, ключи платёжек и одноразовые токены входа.
